...
var hoodie = products.slice();
var tshirt = products.slice();

function filterProductsTshirt() {
    for (var i = 0; i < tshirt.length; i++) {
        if (tshirt[i].type != "t shirt") {
            tshirt.splice(i, 1);
            i--;
        }
    }
...
function filterProductsHoodie() {
    for (var i = 0; i < hoodie.length; i++) {
        if (hoodie[i].type != "hoodie") {
            hoodie.splice(i, 1);
            i--;
        }
    }
...
function displayHoodie() {
    filterProductsHoodie();
    console.log(hoodie);
    for (var i = 0; i < hoodie.length; i++) {
        document.getElementById("mydivH").innerHTML += "<div  id='" + hoodie[i].id + "_holder' class='grid-item '><img src='" + hoodie[i].imgUrl + "'><h4>" + hoodie[i].name + "</h4><p>" + hoodie[i].rating + " <i class='fa fa-star' ></i></p><p>" + hoodie[i].price + " L.E.</p><p>" + hoodie[i].stock + " available</p></div>";
    }
}

function displayTshirt() {
    filterProductsTshirt();
    console.log(tshirt);
    for (var i = 0; i < tshirt.length; i++) {
        document.getElementById("mydivT").innerHTML += "<div  id='" + tshirt[i].id + "_holder' class='grid-item '><img src='" + tshirt[i].imgUrl + "'><h4>" + tshirt[i].name + "</h4><p>" + tshirt[i].rating + " <i class='fa fa-star' ></i></p><p>" + tshirt[i].price + " L.E.</p><p>" + tshirt[i].stock + " available</p></div>";
    }
}